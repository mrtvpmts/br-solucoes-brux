...
export default function Segments() {
    ...
    return (
        <section id="segments" className="relative py-12 md:py-24 bg-transparent overflow-hidden">
            <div className="max-w-7xl mx-auto px-6 md:px-8 relative z-10">

                <div className="flex flex-col items-center mb-8 md:mb-16 space-y-4 md:space-y-6">
                    <h2 className="text-3xl md:text-7xl font-black uppercase text-brand-dark text-center tracking-tighter leading-tight">
                        Excelência em <br />
                        <span className="text-brand-green block mt-2">
                            Cada Segmento
                        </span>
                    </h2>
                    <p className="text-brand-dark/40 font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-[10px] md:text-xs text-center">
                        Segmentos Atendidos pela Engenharia Brux
                    </p>
                    <div className="h-1.5 w-16 md:w-24 bg-brand-green/20 rounded-full" />
                </div>
                ...
                <div className="flex flex-wrap justify-center gap-3 md:gap-10">
                    {segments.map((s, i) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className="industrial-card group flex flex-col gap-3 md:gap-8 cursor-pointer border border-brand-dark/5 bg-white p-5 md:p-12 w-full md:w-[calc(50%-1.25rem)] lg:w-[calc(33.333%-1.7rem)] shadow-sm hover:shadow-xl transition-all"
                        >
                            <div className="text-brand-green/40 group-hover:text-brand-green transition-all duration-500 scale-75 md:scale-100 origin-left">
                                {s.icon}
                            </div>
                            <div className="space-y-1 md:space-y-4">
                                <h3 className="text-sm md:text-2xl font-black uppercase text-brand-dark tracking-widest">{s.title}</h3>
                                <p className="text-brand-dark/60 md:text-brand-dark/80 !text-lg md:!text-2xl font-bold leading-tight line-clamp-3 md:line-clamp-none">
                                    {s.desc}
                                </p>
                            </div>

                            <div className="mt-auto pt-4 md:pt-8 border-t border-brand-dark/5 flex justify-between items-center opacity-30 md:opacity-50 group-hover:opacity-100 transition-opacity">
                ...
                {selectedSegment && (
                    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
                        ...
                        <motion.div
                            ...
                            className="relative w-full max-w-2xl bg-white border border-brand-dark/10 rounded-[40px] p-6 md:p-12 max-h-[85vh] overflow-y-auto shadow-2xl"
                        >
                            <div className="relative z-10 flex flex-col h-full">
                                <div className="flex justify-between items-start mb-12">
                                    <div className="flex gap-6 items-center">
                                        ...
                                        <div>
                                            ...
                                            <h3 className="text-3xl md:text-5xl font-black uppercase text-brand-dark tracking-tighter">{selectedSegment.title}</h3>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => setSelectedSegment(null)}
                                        className="p-3 bg-brand-light hover:bg-brand-dark/5 rounded-full border border-brand-dark/10 text-brand-dark/50 hover:text-brand-dark transition-all"
                                    >
                                        <X size={20} />
                                    </button>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                                    <div className="space-y-8">
                                        <div className="space-y-4">
                                            <div className="flex items-center gap-2 text-brand-green/60">
                                                <Zap size={14} />
                                                <span className="text-[10px] font-black uppercase tracking-widest">Foco Operacional</span>
                                            </div>
                                            <p className="text-brand-dark text-xl font-bold leading-tight uppercase">{selectedSegment.details.focus}</p>
                                        </div>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="p-4 bg-brand-light border border-brand-dark/5 rounded-2xl text-brand-green font-black tracking-widest text-xs uppercase">
                                        ...
                                        <ul className="space-y-4">
                                            {selectedSegment.details.specs.map((spec: string, idx: number) => (
                                                <li key={idx} className="flex items-center gap-4 group">
                                                    ...
                                                    <span className="text-brand-dark/70 text-sm font-medium leading-tight group-hover:text-brand-dark transition-colors">{spec}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                ...
                                <div className="mt-12 pt-8 border-t border-brand-dark/5 flex flex-col md:flex-row justify-between items-center gap-6">
                                    <div className="flex flex-col gap-1 opacity-40 text-[9px] font-black uppercase tracking-[0.2em] text-brand-green">
                                    ...
                                    <button
                                        ...
                                        className="btn-stitch btn-stitch-primary px-8 py-4 text-xs"
                                    >
